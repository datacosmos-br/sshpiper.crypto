...
// underlying connection. Callers must close rw when the client is no
// longer needed to stop the background goroutine and release resources.
// When rw does not implement io.Closer this is not possible, so NewClient
// falls back to fully serializing each request: a single in-flight call at a time.
